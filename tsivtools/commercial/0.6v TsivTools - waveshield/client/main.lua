...
local function buildPlayers(menu)
    ...
    menu:Button('Online Players', 'live player overview', function()
        CreateThread(function()
            ...
            TSIV.ShowBlock({ title = 'Online Players', lines = lines })
        end)
    end)
    ...
    if can('security.view') then
        menu:Button('Suspicious Players', 'Players that have been recently flagged by TsivTools !', function()
            CreateThread(function()
                TSIV.ShowBlock(TSIV.Request('security.detections', { severity = 'all', kind = 'all' }))
            end)
        end)
        menu:Button('Player Security Profile', 'Identity, detections and risk !', function()
            withTarget(function(target)
                CreateThread(function()
                    TSIV.ShowBlock(TSIV.Request('security.profile', { target = target }))
                end)
            end)
        end)
        menu:Button('WaveShield Player Profile', 'WaveShield analysis for a player !', function()
            withTarget(function(target)
                CreateThread(function()
                    ...
                    TSIV.ShowBlock(TSIV.Request('security.waveshield.profile', { identifier = identifier }))
                end)
            end)
            if can('security.waveshield.ban') then
                menu:Button('WaveShield Ban', 'Wave Ban a player !', function()
                    withTarget(function(target)
                        CreateThread(function()
                            local duration = TSIV.InputNumber('WaveShield duration value (0 = permanent)', '0', 10)
                            if duration == nil then return end
                            local reason = TSIV.Input('WaveShield ban reason', '', 150)
                            if reason then
                                TSIV.Action('security.waveshield.ban', {
        ...
        if can('security.monitor') then
            menu:Button('Temporary Monitor player', 'Monitor a player for up to 10 minutes !', function()
                withTarget(function(target)
                    CreateThread(function()
                        local minutes = TSIV.InputNumber('Monitor duration (1-10 minutes)', '10', 2)
                        if minutes then
...
local function buildSecurity(menu)
    ...
    menu:Button('Overview', 'Live security dashboard !', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.status')) end)
    end)
    menu:Button('Live Alerts', 'Live security alerts from TsivTools and WaveShield !', function()
        CreateThread(function()
            local alerts = TSIV.Request('security.alerts')
            local sub = TSIV.Menu.Create('Live Alerts', 'Select an alert for actions !')
                    ...
                    ('%s | risk %d/100'):format(alert.kind, alert.risk), function()
                        ...
                        if can('player.spectate') then
                            actions:Button('Spectate', 'spectate this alert !', function()
                                TSIV.Action('player.spectate', { target = alert.target })
                            end)
                        end
                        TSIV.Menu.Push(actions)
                    end)
    ...
    menu:Button('WaveShield', 'WaveShield integration status !', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.waveshield')) end)
    end)
    menu:Button('Verify WaveShield Server Integration', 'Checks the API credentials ! (if not owner, please report to Dev !!)', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.waveshield.verify')) end)
    end)
    menu:Button('WaveShield Detection Feed', 'Recent logs from Wave !', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.waveshield.logs')) end)
    end)
    menu:Button('WaveShield Online Players', 'Players currently reported by WaveShield !', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.waveshield.players')) end)
    end)
    if can('security.waveshield.ban') then
        menu:Button('WaveShield Ban Player', 'Wave Ban a player !', function()
            withTarget(function(target)
                CreateThread(function()
                    local duration = TSIV.InputNumber('WaveShield duration (0 = permanent)', '0', 10)
                    if duration == nil then return end
                    local reason = TSIV.Input('WaveShield ban reason', '', 150)
                    if reason then
                        TSIV.Action('security.waveshield.ban', {
    ...
    if can('security.waveshield.unban') then
        menu:Button('WaveShield Bans', 'View active WaveShield bans !', function()
            CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.waveshield.bans')) end)
        end)
        menu:Button('WaveShield Unban', 'Remove a Wave ban by ban ID !', function()
            CreateThread(function()
                local banId = TSIV.Input('WaveShield ban ID', '', 128)
                if banId and banId ~= '' then
                    TSIV.Action('security.waveshield.unban', { banId = banId })
                end
            end)
        end)
    end
    menu:Button('WaveShield Health', 'WaveShield, TsivTools and server health !', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.health')) end)
    end)
    ...
    local function feed(title, values, field)
        local sub = TSIV.Menu.Create(title, 'Select a filter and press Enter !')
        for _, value in ipairs(values) do
            sub:Button(value:upper(), 'Print matching detections !!', function()
                CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.detections', { [field] = value })) end)
            end)
        end
        TSIV.Menu.Push(sub)
    end
    ...
    menu:Button('Security Statistics', 'Cheater statistics !', function()
        CreateThread(function() TSIV.ShowBlock(TSIV.Request('security.statistics')) end)
    end)
end
