...
local function trafficState()
    ...
    for key, configKey in pairs(trafficKeys) do
        state[key] = tsivtools.Setting('traffic' .. key, Config.Traffic[configKey]) and true or false
    end
    return state
end
...
tsivtools.RegisterAction('world.traffic', 'world.traffic', function(src, payload)
    ...
    tsivtools.SetSetting('traffic' .. key, state)

    local current = trafficState()
    TriggerClientEvent(tsivtools.Events.run, -1, 'setTraffic', current)

    tsivtools.Notify(src, ('%s are now %s !'):format(key, state and 'OFF' or 'ON'), 'success')
...
tsivtools.RegisterAction('world.cleartraffic', 'world.cleartraffic', function(src)
    local removed = clearTraffic()
    tsivtools.Notify(src, ('Cleared %d traffic entities !'):format(removed), 'success')
    Logs.Staff(src, ('Cleared %d traffic entities'):format(removed))
end)

tsivtools.RegisterAction('prop.spawn', 'prop.spawn', function(src, payload)
    local model = tsivtools.SafeString(payload.model, 48):lower()
    if model == '' then
        tsivtools.Notify(src, 'You need a prop model name !', 'error')
        return
    end
    ...
    run(src, 'spawnProp', { model = model, distance = tsivtools.ToNumber(payload.distance) or 2.5 })
...
local function staffChat(src, text)
    ...
    for _, member in ipairs(tsivtools.GetStaff('mod')) do
        TriggerClientEvent(tsivtools.Events.chat, member.source, line)
    end
...
tsivtools.RegisterAction('staff.announce', 'staff.announce', function(src, payload)
    ...
    TriggerClientEvent(tsivtools.Events.chat, -1, ('^1[ANNOUNCEMENT]^7 %s'):format(message))
...
RegisterCommand('prop', function(src, args)
    if src == 0 or not commandPermission(src, 'prop.spawn') then return end
    local model = tsivtools.SafeString(args[1], 48):lower()
    if model == '' then
    ...
    run(src, 'spawnProp', { model = model, distance = tsivtools.ToNumber(args[2]) or 2.5 })
    Logs.Staff(src, ('Spawned prop %s'):format(model))
end, false)
