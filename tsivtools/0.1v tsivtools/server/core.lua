...
function tsivtools.OutranksTarget(src, target)
    ...
    local mine = tsivtools.RankLevel(tsivtools.GetRank(src))
    local theirs = tsivtools.RankLevel(tsivtools.GetRank(target))
    return mine > theirs
end
...
RegisterNetEvent(tsivtools.Events.action, function(name, payload)
    ...
    if not ok then
        ...
        tsivtools.Notify(src, 'That action failed. Check the server console.', 'error')
    end
end)
